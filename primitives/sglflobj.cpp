		...
		if(Mat)
		{
			...
			if(IgnoreLight)// @dodo nochmal ¸berlegen wie Textur, Material und Farbe sich untereinander verhalten
			{
				...
				else 
				{
					...
					SGLMaterial::MatLoaded=true; //really dirty Hack :-) (sorgt daf¸r, daﬂ er denkt er h‰tte ein Mat. -> DrawVertex heult nich rum)
				}
			}
			else Mat->loadMat();
		}
...
/*	if(Mat && Mat->tex && Mat->tex->valid)
		resetTexKoord();*/
	//nicht mehr nˆtig, da ALLE Texturkoordinaten haben
}
