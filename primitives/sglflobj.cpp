		...
		if(Mat)
		{
			...
			if(IgnoreLight)// @dodo nochmal berlegen wie Textur, Material und Farbe sich untereinander verhalten
			{
				...
				else 
				{
					...
					SGLMaterial::MatLoaded=true; //really dirty Hack :-) (sorgt dafr, da�er denkt er h�te ein Mat. -> DrawVertex heult nich rum)
				}
			}
			else Mat->loadMat();
		}
...
/*	if(Mat && Mat->tex && Mat->tex->valid)
		resetTexKoord();*/
	//nicht mehr n�ig, da ALLE Texturkoordinaten haben
}
