...
bool SGLTextur::Load2DImage(const char *imageFile, bool MipMap)
{
	...
	if(!MipMap)delete buff; //ilLoadImage() mag es scheinbar nicht, wenn man seinen Pfadstring l�cht / hoffentlich l�cht er ihn wenigstens selber
	return ret;
}
...
/*	glGetIntegerv(GL_MAX_3D_TEXTURE_SIZE, &size);
	...
	//ntzt nichts - er glaubt er bek�e die Tex rein bekommt aber unten trotzem "out of memory"
...
{
	glTexEnvf(GL_TEXTURE_ENV,GL_TEXTURE_ENV_MODE,GL_REPLACE);//@todo erstmal Material berschreiben - sp�er w�e bedingtes GL_MODULATE vielleicht besser
...
/*!
	Generiert gltige Werte fr Tiefe Breite und H�e einer Textur.
	
	Wenn die angegebenen Werte keine Potenzen von 2 sind, werden sie durch die n�hstgr�te Zweierpotenz ersetzt.
	Ist ein Wert zu gro� wird er halbiert.
