...
bool ViewTrans::welt2window(const SGLVektor &src,SGLVektor &dst)
{
	if(outDated)
	{
		SGLprintWarning("Implizites Aktualisieren der Viewmatrix 0x%x aus dem Renderer, es ist nicht sicher daß sie korrekt ist",this);
		update();
	}
...
void SGLBaseCam::setView(unsigned int width,unsigned int height)
{
	...
	ViewMatr.outDated=true;//@todo glGetIntegerv(GL_VIEWPORT,view); liefert mist, deshalb von hand - außerdem ändert sich das nur, wenn das fenster geändert wird
}
